...
class pauli_grid():
    ...
    def __init__(self,backend=Aer.get_backend('qasm_simulator_py'),shots=1024,mode='circle'):
        ...
        self.bottom = self.ax.text(-3,1,"",size=11,va='top',color='w')
    ...
    def update_grid(self,rho=None,labels=False,bloch=None,hidden=[],qubit=True,corr=True,message=""):
        ...
        for pauli in self.box:
            if 'I' in pauli:
                ...
                color = self.colors[2]
            self.ax.add_patch( Rectangle( (self.box[pauli][0],self.box[pauli][1]-1), L, L, angle=45, color=color, joinstyle='round') )  
